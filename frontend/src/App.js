...
function App() {
  ...
  useEffect(() => {
    const fetchUserCountryPower = async () => {
      try {
        ...
        // 处理 SQL NullString 格式：{String: "ARE", Valid: true}
        let rawRoleId = currentRole?.role_id || currentRole?.roleId || currentRole?.id || 'IRN';
        // 如果是对象格式，提取 String 字段
        if (typeof rawRoleId === 'object' && rawRoleId !== null) {
          rawRoleId = rawRoleId.Valid ? rawRoleId.String : 'IRN';
        }
        const countryId = rawRoleId.toString().toUpperCase();
        
        // 英文国家名称映射
        const COUNTRY_NAME_EN = {
          'IRN': 'Iran', 'IRQ': 'Iraq', 'SYR': 'Syria', 'ISR': 'Israel',
          'SAU': 'Saudi Arabia', 'USA': 'USA', 'ARE': 'UAE', 'TUR': 'Turkey',
          'EGY': 'Egypt', 'QAT': 'Qatar', 'KWT': 'Kuwait', 'BHR': 'Bahrain',
          'OMN': 'Oman', 'YEM': 'Yemen', 'JOR': 'Jordan', 'LBN': 'Lebanon',
          'PSE': 'Palestine', 'RUS': 'Russia'
        };
        ...
        if (response.ok) {
          ...
          if (userRole) {
            ...
            setUserCountryPower({
              name: userRole.name,
              nameEn: COUNTRY_NAME_EN[countryId] || leader?.locationEn || userRole.name,
  ...
  // 🆕 监听 WebSocket 事件更新（PM Agent 分析完成）
  useEffect(() => {
    const handleEventUpdate = (updatedEvent) => {
      console.log('📡 [App] 收到事件更新:', updatedEvent.id, updatedEvent.pm_analysis ? '(含 PM 分析)' : '');
      setEvents(prevEvents =>
        prevEvents.map(e =>
          e.id === updatedEvent.id ? { ...e, pm_analysis: updatedEvent.pm_analysis } : e
        )
      );
    };

    ws.onEventUpdate(handleEventUpdate);

    return () => {
      // WebSocket 清理在 disconnect 中处理
    };
  }, []);
          ...
          <>
                ...
                <div className="country-power-display">
                  {userCountryPower ? (
                    <>
                      ...
                      <div className="country-stats">
                        ...
                        <div className="stats-row stats-row-2">
                          ...
                          <span className="country-name">{lang === 'zh' ? userCountryPower.name : (userCountryPower.nameEn || userCountryPower.name)}</span>
                          ...
                          <span className="faction-badge-small" style={{
                            ...
                            {lang === 'zh' ? userCountryPower.faction : 
                              userCountryPower.faction === '抵抗轴心' ? 'Resistance Axis' :
                              userCountryPower.faction === '美以联盟' ? 'US-Israel Alliance' :
                              userCountryPower.faction === '温和联盟' ? 'Moderate Alliance' :
                              userCountryPower.faction === '亲穆兄会' ? 'Muslim Brotherhood' : 
                    ...
                    <>
                      ...
                      <div className="country-stats">
                        <div className="stats-row stats-row-1">
                          <span className="loading-text">{lang === 'zh' ? '数据加载中...' : 'Loading...'}</span>
                        </div>
                        <div className="stats-row stats-row-2">
                          <span className="country-name">{lang === 'zh' ? '伊朗' : 'Iran'}</span>
                        </div>
                      </div>
                    </>
                  )}
                        ...
                        <div className="user-details">
                          ...
                          {currentRole && (
                            <div className="user-role">
                              {currentRole.flag || '🏛️'} {lang === 'zh' ? (currentRole.role_name || currentRole.name) : (currentRole.role_name_en || currentRole.nameEn || ({'巴勒斯坦': 'Palestine', '伊拉克': 'Iraq', '卡塔尔': 'Qatar', '以色列': 'Israel', '伊朗': 'Iran', '叙利亚': 'Syria', '沙特阿拉伯': 'Saudi Arabia', '沙特': 'Saudi Arabia', '阿联酋': 'UAE', '土耳其': 'Turkey', '埃及': 'Egypt', '科威特': 'Kuwait', '巴林': 'Bahrain', '阿曼': 'Oman', '也门': 'Yemen', '约旦': 'Jordan', '黎巴嫩': 'Lebanon', '美国': 'USA', '俄罗斯': 'Russia'}[currentRole.role_name || currentRole.name]) || currentRole.role_name || currentRole.name)}
                            </div>
                          )}
                        </div>
            ...
            {showEventPanel && (
              <MemorialModal
                ...
                title={lang === 'zh' ? '📜 完整事件流' : '📜 Event Stream'}
                type="外交"
                width="1000px"
