...
class WorldWebSocket {
  constructor() {
    ...
    if (isDevelopment) {
      ...
      console.log(`[WebSocket] DEV 模式，连接后端 8081: ${wsUrl}`);
    } else {
      // 生产环境：优先用环境变量，次选同源
    ...
    this.ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'public') {
          ...
          // 🆕 事件更新（PM Agent 分析完成）
          console.log('📡 [WebSocket] 收到事件更新:', msg.event?.id);
          this.eventUpdateListeners.forEach(cb => cb(msg.event));
        }
      } catch (err) {
  ...
  onEventUpdate(callback) {
    this.eventUpdateListeners.push(callback);
  }
