...
const WorldChannel = ({ lang = 'zh' }) => {
        ...
        if (response.ok) {
          ...
          if (data.messages && data.messages.length > 0) {
            const historyMessages = data.messages.map(msg => ({
              ...
              type: msg.message_type === 'statement' ? 'public_statement' : 'text'
            }));
            setMessages(historyMessages);
          }
        }
      } catch (err) {
        console.error('❌ [世界频道] 获取历史消息失败:', err);
      }
    };
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.rooms) {
            ...
            setPrivateRooms(roomsMap);
          }
        }
      } catch (err) {
        console.error('❌ [世界频道] 获取聊天室列表失败:', err);
      }
    };
  ...
  const handleCreateRoom = async (roomName) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ [世界频道] 创建聊天室成功:', room.name);
      } else {
        const error = await response.json();
        console.error('❌ [世界频道] 创建聊天室失败:', error.error);
        showToast(error.error || (lang === 'zh' ? '创建聊天室失败' : 'Failed to create room'), 'error');
      }
    } catch (err) {
      console.error('❌ [世界频道] 创建聊天室请求失败:', err);
      showToast(lang === 'zh' ? '创建聊天室请求失败' : 'Request failed', 'error');
    }
  };
  ...
  // Agent 名字到 role_id 的映射
  const agentToRoleMap = {
    'trump': 'usa',
    'netanyahu': 'israel',
    'mujtaba': 'iran',
    'assad': 'syria',
  ...
  const getLeaderInfo = (msg) => {
    ...
    let rawRoleId = msg.role_id || msg.from;
    
    // 如果是 agent 名字（如 trump），转换为 role_id
    if (agentToRoleMap[rawRoleId.toLowerCase()]) {
      rawRoleId = agentToRoleMap[rawRoleId.toLowerCase()];
    }
    ...
    return {
      name: msg.from,
      country: countryMap[roleId]?.[lang] || rawRoleId,
      avatar: null
    };
  };
